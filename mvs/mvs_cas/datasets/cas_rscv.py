...
class MVSDataset(Dataset):
    ...
    def build_list(self):
        ...
        elif self.set_name == 'tianjin':
            sample_list = self.whu_list(data_folder, self.set_name, gt_fext='.png')
        elif self.set_name == 'munchen':
    ...
    def build_all_list(self):
        # Prepare all training samples
        """ generate data paths for all dataset """
    ...
    def read_depth(self, filename, set_name):
        ...
        elif set_name =='tianjin':
            # depimg = imread(filename) # read png depth file
            depimg = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
            depth_image = (np.float32(depimg) / 64.0)  # tianjin dataset

        elif set_name == 'munchen':
            depth_image = np.array(read_pfm(filename)[0], dtype=np.float32)  # munchen dataset
        else:
    ...
    def __getitem__(self, idx):
        ...
        for view in range(view_cnts):
            ...
            elif sat_name == 'munchen':
                cam, _ = self.tr_read_whu_cam(paths[2 * view + 1], self.interval_scale)
            elif sat_name == 'tianjin':
                cam, _ = self.tr_read_whu_cam(paths[2 * view + 1], self.interval_scale)
            else:
        ...
        # fake normal
        normal_image = np.array(np.zeros([3, h, w]), dtype=np.float32)
        normal_ms = {
            "stage1": normal_image,
            "stage2": normal_image,
            "stage3": normal_image,
