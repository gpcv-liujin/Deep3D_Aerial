...
class MVSDataset(Dataset):
    def __init__(self, data_folder, set_name, mode, view_num, interval_scale, normalize):
    ...
    def build_list(self):
        ...
        if self.set_name == 'dtu':
            ...
            sample_list = self.ObliqueWhu_list(data_folder, self.set_name, normal=self.normal, gt_fext='.exr')
    ...
    def ObliqueWhu_list(self, data_folder, set_name, normal=True, gt_fext='.exr'):
        sample_list = []
        ...
        for i in data_cluster:
            ...
            normal_folder = os.path.join(data_folder, ('%s/normals' % i)).replace("\\", "/")
            ...
            for idx in range(len(metas)):
                ...
                normal_image_path = os.path.join(normal_folder, ref_name + gt_fext).replace("\\", "/")
                paths.append(ref_image_path)
                paths.append(ref_cam_path)
                ...
                paths.append(depth_image_path)
                paths.append(normal_image_path)
                sample_list.append((set_name, view_cnts, paths))
    ...
    def tr_read_obliquewhu_cam(self, file, interval_scale=1):
        ...
        extrinsics = np.linalg.inv(extrinsics)  # Tcw  XrightYdown
        cam[0, :, :] = extrinsics
    ...
    def read_depth(self, filename, set_name):
        if set_name =='whu_mvs':
            depimg = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
    ...
    def read_normal(self, filename, set_name):
        if set_name == 'whu_omvs':

            normal_image = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
            normal_image = normal_image[:, :, ::-1]  # inverse channels # [H, W, 3]
            normal_image = normal_image * 2 - 1   # [0, 1] -> [-1, 1]
            normal_image[:, :, 1] *= -1           # XrightYup to XrightYdown
            normal_image[:, :, 2] *= -1

            np_normal = np.array(normal_image, dtype=np.float32)

            normal_mask = np.float32((np_normal >= -1.0) * 1.0) * np.float32((np_normal <= 1.0) * 1.0)
            mask = normal_mask[:, :, 0] * normal_mask[:, :, 1] * normal_mask[:, :, 2]

            np_normal[np_normal < -1.0] = -1.0
            np_normal[np_normal > 1.0] = 1.0

            np_normal_norm = np.linalg.norm(np_normal, ord=2, axis=2, keepdims=True)
            np_normal = np_normal/np_normal_norm

        else:
            raise Exception("{}? Not implemented yet!".format(set_name))

        return np.array(np_normal), np.array(mask)
    ...
    def __getitem__(self, idx):
        ...
        # normal
        normal_image, normal_mask = self.read_normal(os.path.join(paths[2 * view_cnts+1]), sat_name)
        ...
        depth_mask = np.float32((depth_image >= depth_min) * 1.0) * np.float32((depth_image <= depth_max) * 1.0)
        mask = depth_mask * normal_mask
        ...
        normal_ms = {
            "stage1": normal_image.transpose([2, 0, 1]),
            "stage2": normal_image.transpose([2, 0, 1]),
            "stage3": normal_image.transpose([2, 0, 1]),
        }
