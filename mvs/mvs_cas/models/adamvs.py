    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, confidence_map=None):
        ...
        if confidence_map is None:
            ...
            for src_fea, src_proj, confidence in zip(src_features, src_projs, confidence_map):
                ...
                weight_sum = weight_sum + weight.unsqueeze(1)  
                fused_interm = fused_interm + warped_volume2 * weight.unsqueeze(1) 
...
class AdaMVSNet(nn.Module):
    def __init__(self, ndepths=[48, 32, 8], depth_intervals_ratio=[4, 2, 1], share_cr=False, cr_base_chs=[8, 8, 8]):
        ...
        self.feature = FeatureNet(base_channels=8, stride=4, num_stage=self.num_stage) # unet
        self.DepthNet = nn.ModuleList([DepthNet(in_depths=self.ndepths[0], in_channels=self.feature.out_channels[0]),
                                       DepthNet(in_depths=self.ndepths[0], in_channels=self.feature.out_channels[1]),
                                       DepthNet(in_depths=self.ndepths[0], in_up=False, in_channels=self.feature.out_channels[2])])
...
class Infer_AdaMVSNet(nn.Module):
    def __init__(self, num_depth=384, ndepths=[48, 32, 8], depth_intervals_ratio=[4, 2, 1], share_cr=False, cr_base_chs=[8, 8, 8]):
        ...
        self.DepthNet = nn.ModuleList(
            [InferDepthNet(in_depths=self.ndepths[0], in_channels=self.feature.out_channels[0]),
             InferDepthNet(in_depths=self.ndepths[0], in_channels=self.feature.out_channels[1]),
             InferDepthNet(in_depths=self.ndepths[0], in_up=False, in_channels=self.feature.out_channels[2])])
