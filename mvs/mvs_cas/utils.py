...
def compute_metrics_for_each_image3(metric_func):
    def wrapper(normal_error, mask, *args):
        batch_size = normal_error.shape[0]
        results = []
        # compute result one by one
        for idx in range(batch_size):
            ret = metric_func(normal_error[idx], mask[idx], *args)
            results.append(ret)
        return torch.stack(results).mean()
...
@make_nograd_func
def Normal_angles(normal_est, normal_gt, mask=None):
    dot = torch.cosine_similarity(normal_est, normal_gt, dim=1)
    total_angles = torch.acos(torch.clamp(dot, min=-1, max=1)) / np.pi * 180
    return total_angles

@make_nograd_func
@compute_metrics_for_each_image3
def Normalangle_mean_metrics(normal_error_in_angle, mask, thres=None):
    mean = torch.mean(normal_error_in_angle[mask])

    return mean

@make_nograd_func
@compute_metrics_for_each_image3
def Normalangle_median_metrics(normal_error_in_angle, mask, thres=None):
    median = torch.median(normal_error_in_angle[mask])

    return median

@make_nograd_func
@compute_metrics_for_each_image3
def Normalangle_Thres_metrics(normal_error_in_angle, mask, thres):
    assert isinstance(thres, (int, float))
    err_mask = normal_error_in_angle[mask]
    valid_mask = err_mask < thres
    return torch.mean(valid_mask.float())
